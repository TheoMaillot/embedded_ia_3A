...
/* USER CODE BEGIN includes */
 ...
 #define BYTES_IN_FLOATS 32*32*3*4
...
#if !defined(AI_CIFAR10_INPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_in_1[AI_CIFAR10_IN_1_SIZE_BYTES];
ai_i8* data_ins[AI_CIFAR10_IN_NUM] = {
data_in_1
};
#else
ai_i8* data_ins[AI_CIFAR10_IN_NUM] = {
...
#if !defined(AI_CIFAR10_OUTPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_out_1[AI_CIFAR10_OUT_1_SIZE_BYTES];
ai_i8* data_outs[AI_CIFAR10_OUT_NUM] = {
data_out_1
};
#else
ai_i8* data_outs[AI_CIFAR10_OUT_NUM] = {
...
AI_ALIGNED(32)
static uint8_t pool0[AI_CIFAR10_DATA_ACTIVATION_1_SIZE];

ai_handle data_activations0[] = {pool0};

/* AI objects ----------------------------------------------------------------*/

static ai_handle cifar10 = AI_HANDLE_NULL;
...
static int ai_boostrap(ai_handle *act_addr)
{
  ...
  err = ai_cifar10_create_and_init(&cifar10, act_addr, NULL);
  if (err.type != AI_ERROR_NONE) {
    ai_log_err(err, "ai_cifar10_create_and_init");
    return -1;
  }

  ai_input = ai_cifar10_inputs_get(cifar10, NULL);
  ai_output = ai_cifar10_outputs_get(cifar10, NULL);

#if defined(AI_CIFAR10_INPUTS_IN_ACTIVATIONS)
  /*  In the case where "--allocate-inputs" option is used, memory buffer can be
   *  used from the activations buffer. This is not mandatory.
   */
  for (int idx=0; idx < AI_CIFAR10_IN_NUM; idx++) {
   ...
   */
  for (int idx=0; idx < AI_CIFAR10_OUT_NUM; idx++) {
...
static int ai_run(void)
{
  ...
  batch = ai_cifar10_run(cifar10, ai_input, ai_output);
  if (batch != 1) {
    ai_log_err(ai_cifar10_get_error(cifar10),
        "ai_cifar10_run");
    return -1;
  }
...

{
...

    {
...

      {
        ...
        //HAL_UART_Transmit_IT(&huart2, (uint8_t *)tx, sizeof(tx));
        is_synced = 1;
